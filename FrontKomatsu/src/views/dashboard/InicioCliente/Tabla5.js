...
const handlePptFileChange = (event) => {
  ...
  // Datos de ejemplo
  const rows = [
    { id: '4454', firstName: ' 76.176.916-2', lastName: 'SOCIEDAD COMERCIAL CORREA Y CORREA LIMITADA', age: 'Usuario Prueba', montoUSD: '0', fechaini: '12/12/23', fecharech: ' ',asc: '-',
    ac: '✅' ,jfs: '-', jc: '✅',
    sgc: '✅',
    gcc: '✅',
    cfo: '✅',
  ...
  const FileIcon = ({ value, color }) => (
    
    <div style={{ color: color }}>{value === '📄' ? <BiSolidFilePdf /> : ''}</div>
  );
  const PptIcon = ({ value, color }) => (
    
    <div style={{ color: color }}>{value === '📊' ? <BiSolidFileImage /> : ''}</div>
  );
  ...
  const columns = [
    ...
    { field: 'ac', headerName: 'AC',  width: 60,  renderCell: (params) => <CheckIcon value={params.value} color="#439E1C" /> },
    { field: 'jfs', headerName: 'JFC',  width:60  },
    { field: 'jc', headerName: 'JC',  width: 60, renderCell: (params) => <CheckIcon value={params.value} color="#439E1C" /> },
    { field: 'sgc', headerName: 'SGC',  width: 60, renderCell: (params) => <CheckIcon value={params.value} color="#439E1C" /> },
    { field: 'gcc', headerName: 'GCC',  width: 60 , renderCell: (params) => <CheckIcon value={params.value} color="#439E1C" />},
    { field: 'cfo', headerName: 'CFO',  width: 60 , renderCell: (params) => <CheckIcon value={params.value} color="#439E1C" />},
    { field: 'gg', headerName: 'GG',  width: 60 , renderCell: (params) => <CheckIcon value={params.value} color="#439E1C" />},
    { field: 'cfovp', headerName: 'CFO VIP',  width: 60, renderCell: (params) => <CheckIcon value={params.value} color="#439E1C" /> },
    { field: 'ceo', headerName: 'CEO',  width: 60 , renderCell: (params) => <CheckIcon value={params.value} color="#439E1C" />},
   
    { field: 'ppt', headerName: 'PPT',  width: 60 , renderCell: (params) => <PptIcon value={params.value} color="#0D70C9" />},
    { field: 'pdf', headerName: 'PDF',  width: 60, renderCell: (params) => <FileIcon value={params.value} color="#0D70C9" /> },
    {
      field: 'subirpdf',
